...
class Data:
    ...
    def convert_labels(self, from_format="0,1", to_format="-1,1"):
        """
        Converts labels between different formats.
        
        Parameters:
        - from_format (str): Current label format. Supported: "0,1" or "-1,1"
        - to_format (str): Target label format. Supported: "0,1" or "-1,1"
        
        Returns:
        - numpy.ndarray: Converted labels
        """
        if self.y is None:
            raise ValueError("No labels available. Generate or load data first.")
            
        # Make a copy to avoid modifying the original
        converted_y = self.y.copy()
        
        # Convert from 0,1 to -1,1
        if from_format == "0,1" and to_format == "-1,1":
            converted_y = 2 * converted_y - 1
            
        # Convert from -1,1 to 0,1
        elif from_format == "-1,1" and to_format == "0,1":
            converted_y = (converted_y + 1) / 2
            
        # Update the internal labels
        self.y = converted_y
